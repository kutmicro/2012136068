...
void JoystickShield::processEvents() {
    ...
    if (!digitalRead(pin_but_F)) {
        currentButton = DOWN_BUTTON;
    }

    if (!digitalRead(pin_but_G)) {
        currentButton = LEFT_BUTTON;
    }

	if (!digitalRead(pin_but_D) ) {
		currentButton = D_BUTTON;
	}

	if (!digitalRead(pin_but_E)) {
